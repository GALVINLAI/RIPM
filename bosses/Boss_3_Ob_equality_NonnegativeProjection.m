function Boss_3_Ob_equality_NonnegativeProjection
...
% method set = {'RALM','REPM(LQH)','REPM(LSE)','RSQP','RIPM_RepMat','RIPM_CR'};
% RIPM_RepMat uses RepresentMatMethod. 
% RIPM_CR uses KrylovIterMethod. 
...
AllResultBox = zeros(24, n_repeat+1);
...
for n = rdimset
    ...
    for tolKKTres = tolKKTrespowerset
        ...
        for repeat = 1 : n_repeat
            ...
            % for RALM, REPMs.
            options.startingtolgradnorm = max(1e-3, 10^(-tolKKTres + 3)); %%%%%% 1e-2
...
%% sub function
    ...
    function AllResult = Statistics(AllResult)
        ...
        for method_ind=1:6 % data(:,i) = [residual; time; iternum; NLRMrelres];
