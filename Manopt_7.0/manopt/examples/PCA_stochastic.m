...
% 
% PCA (principal component analysis) on a dataset A of size n x d consists
% in solving
% 
%  minimize_X  f(X) = -.5*norm(A*X, 'fro')^2 / n,
% 
% where X is a matrix of dimension d x k with orthonormal columns. This
% is equivalent to finding k dominant (right) singular vectors of A, or k top
% eigenvectors of A'*A.

% n is number of samples, thus each row of A is a date sample.
% d is high dim and number of features of one sample.
% k is low dim
