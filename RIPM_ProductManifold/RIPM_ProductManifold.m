function [xfinal, costfinal, residual, info, options] = RIPM_ProductManifold(problem, x0, options)
...
% 1. MAP_ineq: x maps to <z,g(x)> by fixing z;
% 2. MAP_eq: x maps to <y,h(x)> by fixing y.
%
...
% on the manifold, its cost, and its KKT residual. 
% The output 'info' is a struct-array which contains information about the iterations:
%   iter (integer)
%       The iteration number. The initial value is 0.
%   xcurrent
%       The current point x.
%   xCurCost (double)
...
% log:
%           Nov. 20, 2022: Write the code.
%           Sep. 17, 2023: Modefied  for applying on product manifold.
% 
% 
...
if condet.has_eq_cost
    ...
    egradLagrangian = @(x, y, z) addStructs(addStructs(problem.egrad(x), problem.barHx(x,y)), problem.barGx(x,z));
    ehessLagrangian = @(x, y, z, dx) addStructs(addStructs(problem.ehess(x, dx), problem.ehess_barHx(x, y, dx)), problem.ehess_barGx(x, z, dx));
    ...
    %MODIFIED: For RIPM on ProductManifold
    egradLagrangian = @(x, y, z) addStructs(problem.egrad(x),problem.barGx(x,z));
    ehessLagrangian = @(x, y, z, dx) addStructs(problem.ehess(x, dx),problem.ehess_barGx(x, z, dx));
...
% KKT vector field
    function F = KKTVecField(x, y, z, s)
        ...
        %MODIFIED: For RIPM on ProductManifold
        F.dz = addStructs(problem.ineq_con(x),s);
        F.ds = multiplyStructs(z,s);
...
%MODIFIED: For RIPM on ProductManifold
ineq_size_1 = size(problem.ineq_con(M.rand()).x1);
ineq_size_2 = size(problem.ineq_con(M.rand()).x2);
...
if options.heuristic_z_s
    ...
    %MODIFIED: For RIPM on ProductManifold
    z  =  struct('x1', rand(ineq_size_1),'x2', rand(ineq_size_2));
    s  =  struct('x1', rand(ineq_size_1),'x2', rand(ineq_size_2));
    % z  = rand(ineq_size);	
    % s  = rand(ineq_size);
end
...
problem.dimM = M.dim();
%MODIFIED: For RIPM on ProductManifold
problem.ZeroTxM = M.zerovec(M.rand());
% problem.ZeroTxM = M.zerovec();
if condet.has_eq_cost
    problem.dimEuc_eq = problem.Euc_eq.dim();
    problem.ZeroTyEuc = problem.Euc_eq.zerovec(problem.Euc_eq.rand()); % Do not use zeros(eq_size) because of fixed rank minifold.
    % problem.ZeroTyEuc = problem.Euc_eq.zerovec(); 
else
    problem.dimEuc_eq = [];
    problem.ZeroTyEuc = [];
end
%MODIFIED: For RIPM on ProductManifold
problem.ZeroTzEuc= problem.Euc_ineq.zerovec(problem.Euc_ineq.rand());
ehat  =  struct('x1', ones(ineq_size_1),'x2', ones(ineq_size_2));
...
%MODIFIED: For RIPM on ProductManifold
tau_1 = findMinInStruct(multiplyStructs(z,s))/(innerProductStructs(z,s)/ineq_num);
tau_2 = innerProductStructs(z,s)/realsqrt(PhiCur);
...
rho = innerProductStructs(z,s)/ineq_num;
...
while true
    ...
    %MODIFIED: For RIPM on ProductManifold
    temp1 = multiplyStructs(z, KKTvec.dz);
    temp2 = multiplyStructWithScalar(ehat, sigma*rho);
    temp3 = addStructs(temp1, temp2);
    temp4 = subtractStructs(temp3, KKTvec.ds);
    temp5 = divideStructs(temp4, s);
    cq.dx = M.lincomb(xCur, -1, KKTvec.dx, -1, Gx(xCur, temp5));
    ...
    OpratorTHETA = @(dx) Gx(xCur, multiplyStructs(Gxaj(xCur, dx),(divideStructs(z,s))));
    ...
    % Rcovery dz and ds.
    %MODIFIED: For RIPM on ProductManifold
    temp1 = addStructs(Gxaj(xCur, NTdir.dx), KKTvec.dz);
    temp2 = multiplyStructs(z, temp1);
    temp3 = addStructs(temp2, multiplyStructWithScalar(ehat,sigma*rho));
    NTdir.dz = divideStructs(subtractStructs(temp3, KKTvec.ds),s);
    
    temp4 = subtractStructs(multiplyStructWithScalar(ehat,sigma*rho), KKTvec.ds);
    temp5 = subtractStructs(temp4, multiplyStructs(s, NTdir.dz));
    NTdir.ds = divideStructs(temp5, z);
    ...
    if options.checkNTequation
        ...
        NTdir_error2 = abs(val_innerprodut - 2*(sigma*rho*innerProductStructs(z,s)-PhiCur));
    ...
    % Central functions
    %MODIFIED: For RIPM on ProductManifold
    fun_1 = @(z, s) findMinInStruct(multiplyStructs(z,s))- gamma*tau_1*(innerProductStructs(z,s)/ineq_num);  
    fun_2 = @(z, s, Phi) innerProductStructs(z,s) - gamma*tau_2*realsqrt(Phi);
    ...
    ls_RightItem = 2*(sigma*rho*innerProductStructs(z,s) - PhiCur);
    ...
    while 1
        ...
        %MODIFIED: For RIPM on ProductManifold
        zNew = addStructs(z, multiplyStructWithScalar(NTdir.dz,stepsize));
        sNew = addStructs(s, multiplyStructWithScalar(NTdir.ds,stepsize));
...
% DEBUG ONLY.
    function nablaF = CovarDerivKKT(x, y, z, s, dw)
        ...
        %MODIFIED: For RIPM on ProductManifold
        nablaF.dz = addStructs(Gxaj(x, dw.dx), dw.ds);
        nablaF.ds = addStructs(multiplyStructs(z,dw.ds),multiplyStructs(s,dw.dz));
...
% DEBUG ONLY.
    function nablaFaj = CovarDerivKKTaj(x, y, z, s, dw)
        ...
        %MODIFIED: For RIPM on ProductManifold
        nablaFaj.dz = addStructs(Gxaj(x, dw.dx), multiplyStructs(s, dw.ds));
        nablaFaj.ds = addStructs(multiplyStructs(z, dw.ds), dw.dz);
...
% Note that mus = z, and lambdas = y here.
    function val = KKT_residual(xCur, y, z)
        ...
        %MODIFIED: For RIPM on ProductManifold
        violation = multiplyStructs(z,problem.ineq_con(xCur)); 
        compowvio = normOfStruct(violation)^2;
        ...
        %MODIFIED: For RIPM on ProductManifold
        violation = maxZeroStruct(problem.ineq_con(xCur));       
        ineqpowvio = normOfStruct(violation)^2;
...
% manifold violation 暂时不用
    function manvio = manifoldViolation(xCur) % # No square!
        ...
        elseif contains(M.name(),'Stiefel') %%%%%% # 我添加的！！！！
            [~,k] = size(xCur);
            violation = triu(xCur.'*xCur-eye(k));
            manvio = norm(violation(:),1);
        elseif contains(M.name(),'rank') %% # 移动到这里 fixed-rank manifolds
            try
